...
def getAllStreams(userName):
	...
	for s in userStreams:
		...
		listOfList.append(streamFileToList(s))
...
def printPost (post):
	print("Stream: %sUser: %sDate: %s%s"%(post.stream, post.author, post.date, post.text))

def getUsersCurrentStream(user, stream):
	file = os.popen("./db -getUsersCP %s %s"%(user, stream)).readlines()
...
def updateFile(stream, userName):
	os.popen("./db -update %s %s"%(user,stream))
